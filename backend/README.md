...
### Story Render Jobs (History -> Emotional Story)

#### `POST /stories/render-from-history`
Creates a background render job from history generations with per-character emotional settings.

#### `GET /stories/jobs/{job_id}`
Returns job status, progress, line-level errors, and output audio path (if completed).

#### `GET /stories/{story_id}/audio`
Downloads persisted mixed audio (`final_mix.wav`) generated by the render job.

### Runtime & Security

#### `GET /runtime`
Runtime diagnostics for remote deployment (backend type, device, dtype, host/port).

#### API key protection (optional)
Set `VOICEBOX_API_KEY` and send `Authorization: Bearer <token>` on requests.
When `VOICEBOX_API_KEY` is unset, backend stays open for local development.

For media and SSE GET endpoints where headers may not be available (`<audio>`, avatar images, `EventSource`),
you can also pass:

`?access_token=<VOICEBOX_API_KEY>`

This query token is accepted only for `GET` requests.

Environment variables introduced for remote/Colab profiles:

- `VOICEBOX_API_KEY` (optional)
- `VOICEBOX_ALLOWED_ORIGINS` (CSV, defaults to `*`)
- `VOICEBOX_HOST` (defaults to `127.0.0.1`, or `0.0.0.0` when `VOICEBOX_COLAB_PROFILE=true`)
