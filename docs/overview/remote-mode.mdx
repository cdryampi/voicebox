...
<Steps>
  ...
  <Step title="Set API Key (if enabled)">
    If backend uses `VOICEBOX_API_KEY`, paste the same value in the **API Key** field.
  </Step>

  <Step title="Test Connection">
    Save settings and call `/health` from the app (or run any quick generation) to verify.
  </Step>
</Steps>
...
### Google Colab T4 (quick path)

Use the notebook at `docs/colab/voicebox_t4_deploy.ipynb` to:

- start backend with `VOICEBOX_COLAB_PROFILE=true`
- set `VOICEBOX_API_KEY` for minimal protection
- persist data in Google Drive via `VOICEBOX_DATA_DIR`
- expose temporary public URL through `pyngrok`

## Security Considerations

<Warning>
  For public/tunneled deployments, set `VOICEBOX_API_KEY` and configure the app with the same key.
</Warning>

**Best Practices:**
- Enable API key auth: `VOICEBOX_API_KEY=<your-secret>`
- Use a VPN (WireGuard, Tailscale) instead of exposing to the internet
- Use short-lived tunnel sessions for demos
